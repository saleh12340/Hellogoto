...
class OmniViewModel(private val repository: NoteRepository) : ViewModel() {
    ...
    fun createNewNote() {
        viewModelScope.launch {
            val id = repository.saveNote(Note(title = "ملاحظة جديدة"))
            selectNote(id)
        }
    }

    fun addItem(name: String, quantity: Double, section: String) {
        val noteId = _currentNoteId.value ?: return
        viewModelScope.launch {
            repository.saveItem(NoteItem(noteId = noteId, name = name, quantity = quantity, section = section))
        }
    }
    ...
    fun deleteNote(note: Note) {
        viewModelScope.launch {
            ...
            if (_currentNoteId.value == note.id) {
                _currentNoteId.value = null // Will trigger init logic or we can call createNewNote
            }
        }
    }
}
